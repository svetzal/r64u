 ...
 * @par Example usage:
 * @code
 * StreamControlService *client = new StreamControlService(this);
 * client->setHost("192.168.1.64");
 *
 * connect(client, &StreamControlService::commandSucceeded, this, &MyClass::onSuccess);
 * connect(client, &StreamControlService::commandFailed, this, &MyClass::onError);
 *
 * client->startVideoStream("192.168.1.100", 21000);
 * @endcode
 */
class StreamControlService : public IStreamControlService
{
    Q_OBJECT
...
public:
     ...
     */
    explicit StreamControlService(QObject *parent = nullptr);

    /**
     * @brief Destructor. Closes any open connection.
     */
    ~StreamControlService() override;
