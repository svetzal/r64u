 ...
 * @brief Unit tests for StreamControlService — the imperative TCP shell around
...
private slots:
    void init() { client_ = new StreamControlService(this); }
    ...
    {
        QSignalSpy spy(client_, &StreamControlService::commandFailed);
    ...
    void testClearPendingCommands_PreventsQueuedSend()
    {
        ...
        QSignalSpy succeededSpy(client_, &StreamControlService::commandSucceeded);
        QTest::qWait(50);
...
private:
    StreamControlService *client_ = nullptr;
};

QTEST_MAIN(TestStreamControlClient)
#include "test_streamcontrolservice.moc"
