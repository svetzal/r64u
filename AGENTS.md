...
**Domain signals vs error signals:** Some intermediate classes use `*Failed` signals purely as state-transition or funneling signals within a subsystem — not as independent error paths. These are permitted **only** when the entire chain terminates at exactly one `errorReported()` boundary that is registered with `ErrorHandler`. Examples: `FolderOperationCoordinator::operationFailed`, `TransferFtpHandler::operationFailed`, `TransferDispatchHandler::operationFailed`, `TransferDeleteHandler::operationFailed`, `SingleFileEnqueueHandler::operationFailed`, `TransferManager::operationFailed`, `TransferQueue::operationFailed`, `RecursiveScanCoordinator::downloadDirectoryFailed` (via `TransferManager::operationFailed`), and `RemoteListingCoordinator::listingFailed` all funnel through `TransferService::operationFailed → errorReported`. Do NOT register any of these intermediate emitters directly with `ErrorHandler` — that would double-report the same error to the user.

`IFtpClient::operationFailed(operation, remotePath, localPath, message)` is a correlation signal emitted right after `IFtpClient::error()` when a failure ends a specific request. The FTP client is shared (remote browser, previews, playlists, config loader, transfer queue), so components that must react only to their own requests (e.g. `TransferFtpHandler`) listen to it instead of `error()`. It is never an error path of its own: the user sees the failure once, via `error() → errorReported`.
