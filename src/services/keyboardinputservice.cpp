...
void KeyboardInputService::sendPetscii(quint8 petscii)
{
    ...
    restClient_->writeMem(QString("%1").arg(KeyboardBufferAddress, 4, 16, QChar('0')), bufferData);

    // Set buffer length to 1 at $00C6
    QByteArray lengthData;
    lengthData.append(static_cast<char>(1));
    restClient_->writeMem(QString("%1").arg(BufferLengthAddress, 4, 16, QChar('0')), lengthData);
...
quint8 KeyboardInputService::asciiToPetscii(char ascii)
{
    ...
    case ']':
        return 93;
    case '^':
        return 94;  // Up arrow in PETSCII
    case '_':
        return 95;  // Left arrow in PETSCII
    default:
        break;
    }
