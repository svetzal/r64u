...
private slots:
    ...
    void testDownloadDatabase_callsDownloaderWithCorrectUrl()
    {
        ...
        SonglengthsDatabaseService db(&mock);

        db.downloadDatabase();

        QCOMPARE(mock.mockDownloadCallCount(), 1);
        QCOMPARE(mock.mockLastUrl(),
                 QUrl(QString::fromLatin1(SonglengthsDatabaseService::DatabaseUrl)));
    }
    ...
    void testDownloadProgress_forwardsSignal()
    {
        ...
        QSignalSpy spy(&db, &SonglengthsDatabaseService::downloadProgress);
    ...
    void testDownloadFinished_parsesAndEmitsSignals()
    {
        ...
        QSignalSpy finishedSpy(&db, &SonglengthsDatabaseService::downloadFinished);
        QSignalSpy loadedSpy(&db, &SonglengthsDatabaseService::databaseLoaded);
    ...
    void testDownloadFinished_emptyData_emitsFailure()
    {
        ...
        QSignalSpy failedSpy(&db, &SonglengthsDatabaseService::downloadFailed);
        QSignalSpy finishedSpy(&db, &SonglengthsDatabaseService::downloadFinished);

        db.downloadDatabase();
        // The mock already blocks empty data at the HttpFileDownloaderService level,
    ...
    void testDownloadFailed_forwardsError()
    {
        ...
        QSignalSpy spy(&db, &SonglengthsDatabaseService::downloadFailed);
...
#include "test_songlengthsdatabaseservice.moc"
