 ...
 */
const char petscii::PetsciiConverter::petsciiToAsciiTable[256] = {
 ...
 * Based on Ultimate64/Ultimate-II Control Library for Amiga OS 3.x
 */
const quint8 petscii::PetsciiConverter::asciiToPetsciiTable[128] = {
    // 0x00-0x1F: Control codes
    0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0D, 0x0B, 0x0C, 0x0D, 0x0E,
    0x0F,  // \n (0x0A) -> RETURN (0x0D)
...
QString petscii::PetsciiConverter::toAscii(const QByteArray &data, bool stopAtPadding)
{
    QString result;
    result.reserve(data.size());

    for (const auto byte : data) {
        auto petsciiVal = static_cast<quint8>(byte);

        // $A0 is shift-space (padding character in filenames)
        if (stopAtPadding && petsciiVal == 0xA0) {
            break;
        }
        ...
        char ascii = petsciiToAsciiTable[petsciiVal];
        if (ascii != 0) {
            result += QChar(ascii);
        }
...
char petscii::PetsciiConverter::toAscii(quint8 petsciiVal)
{
    return petsciiToAsciiTable[petsciiVal];
}

QString petscii::PetsciiConverter::toDisplayString(const QByteArray &data)
{
    ...
    for (const auto byte : data) {
        ...
        result += QChar(0xE000 + petsciiVal);
    }

    return result;
}

QByteArray petscii::PetsciiConverter::fromAscii(const QString &text)
{
    QByteArray result;
...
quint8 petscii::PetsciiConverter::fromAscii(char ascii)
{
...
bool petscii::PetsciiConverter::isPrintable(quint8 petsciiVal)
{
    // Printable: 0x20-0x7F and 0xA0-0xFF (excluding pure control codes)
    if (petsciiVal >= 0x20 && petsciiVal <= 0x7F) {
        return true;
    }
    if (petsciiVal >= 0xA0) {
        return true;
...
bool petscii::PetsciiConverter::isGraphics(quint8 petsciiVal)
{
    // Graphics characters:
    ...
    if (petsciiVal >= 0x60 && petsciiVal <= 0x7F) {
        return true;
    }
    if (petsciiVal >= 0xA0 && petsciiVal <= 0xBF) {
        return true;
    }
    if (petsciiVal >= 0xE0) {
        return true;
...
bool petscii::PetsciiConverter::isControl(quint8 petsciiVal)
{
    // Control codes: 0x00-0x1F and 0x80-0x9F
    if (petsciiVal <= 0x1F) {
        return true;
    }
    if (petsciiVal >= 0x80 && petsciiVal <= 0x9F) {
        return true;
    }
    return false;
}
